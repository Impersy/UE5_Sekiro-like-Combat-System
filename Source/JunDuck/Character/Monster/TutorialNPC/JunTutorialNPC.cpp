...
void AJunTutorialNPC::ApplyJumpCounterStompResult(
	AJunPlayer* Player,
	...
	float PostureAmount)
{
	Super::ApplyJumpCounterStompResult(Player, HitReactType, DamageData, DefenseKnockbackData, PostureAmount);
	NotifyTutorialJumpCounterStompSuccess(Player);
}
...
EJunDefenseOutcome AJunTutorialNPC::TryHandleIncomingHitBeforeDamage(
	...
	const FJunAttackDefenseRuleData& DefenseRuleData)
{
	if (!bTutorialCombatEnabled)
	{
		return EJunDefenseOutcome::Rejected;
	}
...
void AJunTutorialNPC::NotifyPlayerParrySucceeded(AJunPlayer* Parrier, bool bPerfectParry, bool bAirParry)
{
	NotifyTutorialParrySuccess(Parrier, bAirParry);
}
...
bool AJunTutorialNPC::NotifyMikiriCounteredBy(AJunPlayer* CounterPlayer)
{
	...
	NotifyTutorialMikiriSuccess(CounterPlayer);
	return bHandled;
}
...
void AJunTutorialNPC::OnLockedOnBy(AJunPlayer* Player)
{
	NotifyTutorialLockOn(Player);
}
...
bool AJunTutorialNPC::ShouldPreventPlayerDeathForTutorial(const AJunPlayer* Player) const
{
	return Player && Player == CurrentTarget && IsTutorialInProgress();
}

void AJunTutorialNPC::NotifyPlayerDrinkPotionForTutorial(AJunPlayer* Player)
{
	NotifyTutorialDrinkPotion(Player);
}
...
void AJunTutorialNPC::ApplyTutorialTaskMode()
{
	...
	case EJunTutorialTaskType::AirParryOnce:
		if (AJunPlayer* Player = Cast<AJunPlayer>(CurrentTarget.Get()))
		{
			if (AJunPlayerController* PlayerController = Cast<AJunPlayerController>(Player->GetController()))
			{
				PlayerController->ShowAirParryTutorialGuide();
			}
		}
		SetTutorialMode(EJunTutorialNPCMode::AttackTraining);
		break;
