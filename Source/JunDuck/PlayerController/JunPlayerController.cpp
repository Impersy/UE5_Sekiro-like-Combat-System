...
void AJunPlayerController::BeginPlay()
{
	...
	BeginLevelIntroTransition();
	InitializeCombatWidgets();
	if (!CombatHUDWidget)
	{
		CompleteLevelIntroTransition();
	}
}
...
void AJunPlayerController::SetupInputComponent()
{
	...
	FInputKeyBinding& PauseBinding = InputComponent->BindKey(EKeys::Escape, IE_Pressed, this, &ThisClass::TogglePauseMenu);
...
void AJunPlayerController::UpdateBossMapNotice()
{
	if (!bBossMapNoticeCheckActive || bLevelIntroInputLocked || !CombatHUDWidget || CombatHUDWidget->IsMapNoticeActive())
	{
		return;
	}
	...
	for (TActorIterator<ANavMeshBoundsVolume> It(GetWorld()); It; ++It)
	{
		...
		if (!NavVolume->ActorHasTag(TEXT("MapNotice.Boss")))
		{
			continue;
		}

		const FBox VolumeBounds = NavVolume->GetComponentsBoundingBox(true);
		const FVector PlayerLocation = JunPlayer->GetActorLocation();
		if (!bBossMapNoticeVolumeLogged)
		{
			bBossMapNoticeVolumeLogged = true;
			UE_LOG(LogTemp, Log, TEXT("MapNotice | Boss volume found. Actor=%s BoundsMin=%s BoundsMax=%s"),
				*NavVolume->GetName(), *VolumeBounds.Min.ToCompactString(), *VolumeBounds.Max.ToCompactString());
		}

		if (VolumeBounds.IsValid && VolumeBounds.IsInsideOrOn(PlayerLocation))
		{
			bBossMapNoticeCheckActive = false;
			UE_LOG(LogTemp, Log, TEXT("MapNotice | Player entered boss volume. Player=%s"),
				*PlayerLocation.ToCompactString());
			CombatHUDWidget->ShowMapNotice(2);
		}
		break;
	}
}

void AJunPlayerController::TogglePauseMenu()
{
	if (bLevelIntroInputLocked)
	{
		return;
	}
	...
	if (CombatHUDWidget)
	{
		InputMode.SetWidgetToFocus(CombatHUDWidget->TakeWidget());
	}
	SetInputMode(InputMode);
}

void AJunPlayerController::ClosePauseMenu()
{
	bPauseMenuOpen = false;
	if (CombatHUDWidget)
	{
		CombatHUDWidget->HidePauseMenu();
	}
	SetPause(false);
	bShowMouseCursor = false;
...
void AJunPlayerController::RestartPausedGame()
{
	bPauseMenuOpen = false;
	SetPause(false);
	bShowMouseCursor = false;
...
void AJunPlayerController::BeginLevelIntroTransition()
{
	bLevelIntroInputLocked = true;
	bShowMouseCursor = false;
...
void AJunPlayerController::CompleteLevelIntroTransition()
{
	if (!bLevelIntroInputLocked)
	{
		return;
...
void AJunPlayerController::QuitPausedGame()
{
...
void AJunPlayerController::StartRespawnFullBlackFadeOut()
{
	if (CombatHUDWidget)
	{
		CombatHUDWidget->StartRespawnFadeOut();
	}
}
...
void AJunPlayerController::ShowAirParryTutorialGuide()
{
	if (!BeginTutorialGuidePause())
	{
		return;
	...
	CombatHUDWidget->ShowAirParryGuide();
}
